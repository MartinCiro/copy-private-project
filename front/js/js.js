const urlBase = "http://localhost:4000/";

const funcionData = (url, metodo, headers = {}, body = null) => {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    xhr.open(metodo, url, true);

    // Configurar los headers
    for (const key in headers) {
      xhr.setRequestHeader(key, headers[key]);
    }

    xhr.onreadystatechange = function() {
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            const responseJson = JSON.parse(xhr.responseText);
            resolve(responseJson);
          } catch (err) {
            reject(`Error parsing JSON: ${err}`);
          }
        } else {
          reject(`HTTP error! status: ${xhr.status}`);
        }
      }
    };

    xhr.onerror = function() {
      reject('Network error');
    };

    if (body != null) {
      // Convertir body a JSON si es un objeto
      if (typeof body === 'object') {
        body = JSON.stringify(body);
      }
    }

    xhr.send(body);
  });
};

const obtenerDatos = async (url, metodo, headers = {}, body = null) => {
  return await funcionData(url, metodo, headers, body);
};

let games;

$(document).ready(async function () {
  try {
    setTimeout(async function () {
      let data1 = await obtenerDatos(`${urlBase}gameAPI`, 'GET', {
        'Accept': 'application/json',
      });
  
      /* const promises = data1.result.map(async (element) => {
        setTimeout(async function () {
          return await obtenerDatos(`${urlBase}dataJuego`, 'POST', {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
          }, {
            'id': element.id,
            'uuid': element.uid,
          });
        }, 5000)
      });
  
      const result = await Promise.all(promises);
      const vectorResult = [];
      result.forEach(element => {
        console.log(element);
        element.result.includes("No se ha encontrado el juego")? element : vectorResult.push(element.result);
      });
      console.log(vectorResult) */
    }, 1500);
  } catch (error) {
    console.error('Error:', error);
  }
});




/* 

const obtenerDatosDeJuegos = async (idGame) => {
  const urlBase = `https://api.rawg.io/api/games/${idGame}?key=4f002dc7d0ca4bd88f65d35100aa1bba` // Reemplaza con tu clave de API real
  data=await fetch(urlBase).then(res => res.text()).then(text => JSON.parse(text))
  data2=await funcionData(`${urlBase}`, 'GET')
  return data2
};
const imageElement = document.getElementById("slide"); // Get the image element
categorys = [
  "Action",
  "Indie",
  "Adventure",
  "RPG",
  "Strategy",
  "Shooter",
  "Casual",
  "Simulation",
  "Puzzle",
  "Arcade",
  "Platformer",
  "Racing",
  "Massively Multiplayer",
  "Sports",
  "Fighting",
  "Family",
  "Board Games",
  "Educational",
  "Card",
];

data = {
  count: 19,
  next: null,
  previous: null,
  results: [
    {
      id: 4,
      name: "Action",
      slug: "action",
      games_count: 179922,
      image_background:
        "https://media.rawg.io/media/games/6fc/6fcf4cd3b17c288821388e6085bb0fc9.jpg",
      games: [
        {
          id: 3498,
          slug: "grand-theft-auto-v",
          name: "Grand Theft Auto V",
          added: 20901,
        },
        {
          id: 3328,
          slug: "the-witcher-3-wild-hunt",
          name: "The Witcher 3: Wild Hunt",
          added: 20340,
        },
        {
          id: 5286,
          slug: "tomb-raider",
          name: "Tomb Raider (2013)",
          added: 16607,
        },
        {
          id: 13536,
          slug: "portal",
          name: "Portal",
          added: 16372,
        },
        {
          id: 12020,
          slug: "left-4-dead-2",
          name: "Left 4 Dead 2",
          added: 16260,
        },
        {
          id: 5679,
          slug: "the-elder-scrolls-v-skyrim",
          name: "The Elder Scrolls V: Skyrim",
          added: 15747,
        },
      ],
    },
    {
      id: 51,
      name: "Indie",
      slug: "indie",
      games_count: 65031,
      image_background:
        "https://media.rawg.io/media/games/942/9424d6bb763dc38d9378b488603c87fa.jpg",
      games: [
        {
          id: 1030,
          slug: "limbo",
          name: "Limbo",
          added: 13383,
        },
        {
          id: 422,
          slug: "terraria",
          name: "Terraria",
          added: 12480,
        },
        {
          id: 3272,
          slug: "rocket-league",
          name: "Rocket League",
          added: 12284,
        },
        {
          id: 9767,
          slug: "hollow-knight",
          name: "Hollow Knight",
          added: 10825,
        },
        {
          id: 3612,
          slug: "hotline-miami",
          name: "Hotline Miami",
          added: 10305,
        },
        {
          id: 3790,
          slug: "outlast",
          name: "Outlast",
          added: 10259,
        },
      ],
    },
    {
      id: 3,
      name: "Adventure",
      slug: "adventure",
      games_count: 139540,
      image_background:
        "https://media.rawg.io/media/games/364/3642d850efb217c58feab80b8affaa89.jpg",
      games: [
        {
          id: 3439,
          slug: "life-is-strange-episode-1-2",
          name: "Life is Strange",
          added: 15052,
        },
        {
          id: 23027,
          slug: "the-walking-dead",
          name: "The Walking Dead: Season 1",
          added: 11120,
        },
        {
          id: 13668,
          slug: "amnesia-the-dark-descent",
          name: "Amnesia: The Dark Descent",
          added: 9931,
        },
        {
          id: 19487,
          slug: "alan-wake",
          name: "Alan Wake",
          added: 9902,
        },
        {
          id: 4386,
          slug: "saints-row-the-third",
          name: "Saints Row: The Third",
          added: 9807,
        },
        {
          id: 29177,
          slug: "detroit-become-human",
          name: "Detroit: Become Human",
          added: 9708,
        },
      ],
    },
    {
      id: 5,
      name: "RPG",
      slug: "role-playing-games-rpg",
      games_count: 55931,
      image_background:
        "https://media.rawg.io/media/games/d0f/d0f91fe1d92332147e5db74e207cfc7a.jpg",
      games: [
        {
          id: 3328,
          slug: "the-witcher-3-wild-hunt",
          name: "The Witcher 3: Wild Hunt",
          added: 20340,
        },
        {
          id: 5679,
          slug: "the-elder-scrolls-v-skyrim",
          name: "The Elder Scrolls V: Skyrim",
          added: 15747,
        },
        {
          id: 802,
          slug: "borderlands-2",
          name: "Borderlands 2",
          added: 15012,
        },
        {
          id: 3070,
          slug: "fallout-4",
          name: "Fallout 4",
          added: 13352,
        },
        {
          id: 41494,
          slug: "cyberpunk-2077",
          name: "Cyberpunk 2077",
          added: 12659,
        },
        {
          id: 278,
          slug: "horizon-zero-dawn",
          name: "Horizon Zero Dawn",
          added: 12452,
        },
      ],
    },
    {
      id: 10,
      name: "Strategy",
      slug: "strategy",
      games_count: 56233,
      image_background:
        "https://media.rawg.io/media/games/be9/be9cf02720c9326e11d0fda14518554f.jpg",
      games: [
        {
          id: 10243,
          slug: "company-of-heroes-2",
          name: "Company of Heroes 2",
          added: 9324,
        },
        {
          id: 13633,
          slug: "civilization-v",
          name: "Sid Meier's Civilization V",
          added: 9279,
        },
        {
          id: 11147,
          slug: "ark-survival-of-the-fittest",
          name: "ARK: Survival Of The Fittest",
          added: 8194,
        },
        {
          id: 10065,
          slug: "cities-skylines",
          name: "Cities: Skylines",
          added: 8164,
        },
        {
          id: 13910,
          slug: "xcom-enemy-unknown",
          name: "XCOM: Enemy Unknown",
          added: 8155,
        },
        {
          id: 5525,
          slug: "brutal-legend",
          name: "Brutal Legend",
          added: 8088,
        },
      ],
    },
    {
      id: 2,
      name: "Shooter",
      slug: "shooter",
      games_count: 59479,
      image_background:
        "https://media.rawg.io/media/games/587/587588c64afbff80e6f444eb2e46f9da.jpg",
      games: [
        {
          id: 4200,
          slug: "portal-2",
          name: "Portal 2",
          added: 19226,
        },
        {
          id: 4291,
          slug: "counter-strike-global-offensive",
          name: "Counter-Strike: Global Offensive",
          added: 16901,
        },
        {
          id: 12020,
          slug: "left-4-dead-2",
          name: "Left 4 Dead 2",
          added: 16260,
        },
        {
          id: 4062,
          slug: "bioshock-infinite",
          name: "BioShock Infinite",
          added: 15218,
        },
        {
          id: 802,
          slug: "borderlands-2",
          name: "Borderlands 2",
          added: 15012,
        },
        {
          id: 13537,
          slug: "half-life-2",
          name: "Half-Life 2",
          added: 14551,
        },
      ],
    },
    {
      id: 40,
      name: "Casual",
      slug: "casual",
      games_count: 53116,
      image_background:
        "https://media.rawg.io/media/games/11f/11fd681c312c14644ab360888dba3486.jpg",
      games: [
        {
          id: 9721,
          slug: "garrys-mod",
          name: "Garry's Mod",
          added: 9694,
        },
        {
          id: 326292,
          slug: "fall-guys",
          name: "Fall Guys: Ultimate Knockout",
          added: 8500,
        },
        {
          id: 9830,
          slug: "brawlhalla",
          name: "Brawlhalla",
          added: 7583,
        },
        {
          id: 356714,
          slug: "among-us",
          name: "Among Us",
          added: 7221,
        },
        {
          id: 1959,
          slug: "goat-simulator",
          name: "Goat Simulator",
          added: 6230,
        },
        {
          id: 16343,
          slug: "a-story-about-my-uncle",
          name: "A Story About My Uncle",
          added: 5875,
        },
      ],
    },
    {
      id: 14,
      name: "Simulation",
      slug: "simulation",
      games_count: 69437,
      image_background:
        "https://media.rawg.io/media/games/6d3/6d33014a4ed48a19c30a77ead5a0f62e.jpg",
      games: [
        {
          id: 10035,
          slug: "hitman",
          name: "Hitman",
          added: 10448,
        },
        {
          id: 654,
          slug: "stardew-valley",
          name: "Stardew Valley",
          added: 9936,
        },
        {
          id: 9721,
          slug: "garrys-mod",
          name: "Garry's Mod",
          added: 9694,
        },
        {
          id: 9882,
          slug: "dont-starve-together",
          name: "Don't Starve Together",
          added: 9198,
        },
        {
          id: 22509,
          slug: "minecraft",
          name: "Minecraft",
          added: 8304,
        },
        {
          id: 10065,
          slug: "cities-skylines",
          name: "Cities: Skylines",
          added: 8164,
        },
      ],
    },
    {
      id: 7,
      name: "Puzzle",
      slug: "puzzle",
      games_count: 97277,
      image_background:
        "https://media.rawg.io/media/games/74d/74dafeb9a442b87b9dd4a1d4a2faa37b.jpg",
      games: [
        {
          id: 4200,
          slug: "portal-2",
          name: "Portal 2",
          added: 19226,
        },
        {
          id: 13536,
          slug: "portal",
          name: "Portal",
          added: 16372,
        },
        {
          id: 1030,
          slug: "limbo",
          name: "Limbo",
          added: 13383,
        },
        {
          id: 19709,
          slug: "half-life-2-episode-two",
          name: "Half-Life 2: Episode Two",
          added: 10890,
        },
        {
          id: 1450,
          slug: "inside",
          name: "INSIDE",
          added: 7874,
        },
        {
          id: 3853,
          slug: "trine-2-complete-story",
          name: "Trine 2: Complete Story",
          added: 7144,
        },
      ],
    },
    {
      id: 11,
      name: "Arcade",
      slug: "arcade",
      games_count: 22657,
      image_background:
        "https://media.rawg.io/media/games/370/3703c683968a54f09630dcf03366ea35.jpg",
      games: [
        {
          id: 3612,
          slug: "hotline-miami",
          name: "Hotline Miami",
          added: 10305,
        },
        {
          id: 17540,
          slug: "injustice-gods-among-us-ultimate-edition",
          name: "Injustice: Gods Among Us Ultimate Edition",
          added: 9405,
        },
        {
          id: 22509,
          slug: "minecraft",
          name: "Minecraft",
          added: 8304,
        },
        {
          id: 4003,
          slug: "grid-2",
          name: "GRID 2",
          added: 7386,
        },
        {
          id: 3408,
          slug: "hotline-miami-2-wrong-number",
          name: "Hotline Miami 2: Wrong Number",
          added: 6046,
        },
        {
          id: 58753,
          slug: "forza-horizon-4",
          name: "Forza Horizon 4",
          added: 5907,
        },
      ],
    },
    {
      id: 83,
      name: "Platformer",
      slug: "platformer",
      games_count: 100779,
      image_background:
        "https://media.rawg.io/media/games/388/388935d851846f8ec747fffc7c765800.jpg",
      games: [
        {
          id: 1030,
          slug: "limbo",
          name: "Limbo",
          added: 13383,
        },
        {
          id: 422,
          slug: "terraria",
          name: "Terraria",
          added: 12480,
        },
        {
          id: 9767,
          slug: "hollow-knight",
          name: "Hollow Knight",
          added: 10825,
        },
        {
          id: 41,
          slug: "little-nightmares",
          name: "Little Nightmares",
          added: 10751,
        },
        {
          id: 3144,
          slug: "super-meat-boy",
          name: "Super Meat Boy",
          added: 9281,
        },
        {
          id: 17572,
          slug: "batman-aa-goty",
          name: "Batman: Arkham Asylum Game of the Year Edition",
          added: 8011,
        },
      ],
    },
    {
      id: 1,
      name: "Racing",
      slug: "racing",
      games_count: 24729,
      image_background:
        "https://media.rawg.io/media/games/0c1/0c1c9965ba59166ab986a663ab2252dc.jpg",
      games: [
        {
          id: 3272,
          slug: "rocket-league",
          name: "Rocket League",
          added: 12284,
        },
        {
          id: 4003,
          slug: "grid-2",
          name: "GRID 2",
          added: 7386,
        },
        {
          id: 2572,
          slug: "dirt-rally",
          name: "DiRT Rally",
          added: 6680,
        },
        {
          id: 58753,
          slug: "forza-horizon-4",
          name: "Forza Horizon 4",
          added: 5907,
        },
        {
          id: 5578,
          slug: "grid",
          name: "GRID (2008)",
          added: 5317,
        },
        {
          id: 19491,
          slug: "burnout-paradise-the-ultimate-box",
          name: "Burnout Paradise: The Ultimate Box",
          added: 4655,
        },
      ],
    },
    {
      id: 59,
      name: "Massively Multiplayer",
      slug: "massively-multiplayer",
      games_count: 3621,
      image_background:
        "https://media.rawg.io/media/games/596/596a48ef3b62b63b4cc59633e28be903.jpg",
      games: [
        {
          id: 10213,
          slug: "dota-2",
          name: "Dota 2",
          added: 12327,
        },
        {
          id: 766,
          slug: "warframe",
          name: "Warframe",
          added: 12238,
        },
        {
          id: 10533,
          slug: "path-of-exile",
          name: "Path of Exile",
          added: 9886,
        },
        {
          id: 10142,
          slug: "playerunknowns-battlegrounds",
          name: "PlayerUnknown’s Battlegrounds",
          added: 9805,
        },
        {
          id: 362,
          slug: "for-honor",
          name: "For Honor",
          added: 9273,
        },
        {
          id: 326292,
          slug: "fall-guys",
          name: "Fall Guys: Ultimate Knockout",
          added: 8500,
        },
      ],
    },
    {
      id: 15,
      name: "Sports",
      slug: "sports",
      games_count: 21415,
      image_background:
        "https://media.rawg.io/media/games/b59/b59560a7277b16b53e4786b4abe45baa.jpg",
      games: [
        {
          id: 3272,
          slug: "rocket-league",
          name: "Rocket League",
          added: 12284,
        },
        {
          id: 326292,
          slug: "fall-guys",
          name: "Fall Guys: Ultimate Knockout",
          added: 8500,
        },
        {
          id: 2572,
          slug: "dirt-rally",
          name: "DiRT Rally",
          added: 6680,
        },
        {
          id: 53341,
          slug: "jet-set-radio-2012",
          name: "Jet Set Radio",
          added: 5094,
        },
        {
          id: 9575,
          slug: "vrchat",
          name: "VRChat",
          added: 4651,
        },
        {
          id: 715,
          slug: "steep",
          name: "Steep",
          added: 3961,
        },
      ],
    },
    {
      id: 6,
      name: "Fighting",
      slug: "fighting",
      games_count: 11753,
      image_background:
        "https://media.rawg.io/media/games/aac/aac683272f862540a18625f02f5f3679.jpg",
      games: [
        {
          id: 5286,
          slug: "tomb-raider",
          name: "Tomb Raider (2013)",
          added: 16607,
        },
        {
          id: 17540,
          slug: "injustice-gods-among-us-ultimate-edition",
          name: "Injustice: Gods Among Us Ultimate Edition",
          added: 9405,
        },
        {
          id: 108,
          slug: "mortal-kombat-x",
          name: "Mortal Kombat X",
          added: 8644,
        },
        {
          id: 28179,
          slug: "sega-mega-drive-and-genesis-classics",
          name: "SEGA Mega Drive and Genesis Classics",
          added: 8009,
        },
        {
          id: 9830,
          slug: "brawlhalla",
          name: "Brawlhalla",
          added: 7583,
        },
        {
          id: 274480,
          slug: "mortal-kombat-11",
          name: "Mortal Kombat 11",
          added: 5352,
        },
      ],
    },
    {
      id: 19,
      name: "Family",
      slug: "family",
      games_count: 5395,
      image_background:
        "https://media.rawg.io/media/games/793/79307648580a262f6cac402c3007efe0.jpg",
      games: [
        {
          id: 3254,
          slug: "journey",
          name: "Journey",
          added: 8323,
        },
        {
          id: 3729,
          slug: "lego-the-hobbit",
          name: "LEGO The Hobbit",
          added: 5009,
        },
        {
          id: 3350,
          slug: "broken-age",
          name: "Broken Age",
          added: 4971,
        },
        {
          id: 1259,
          slug: "machinarium",
          name: "Machinarium",
          added: 4474,
        },
        {
          id: 1140,
          slug: "world-of-goo",
          name: "World of Goo",
          added: 4389,
        },
        {
          id: 4331,
          slug: "sonic-generations",
          name: "Sonic Generations",
          added: 4131,
        },
      ],
    },
    {
      id: 28,
      name: "Board Games",
      slug: "board-games",
      games_count: 8375,
      image_background:
        "https://media.rawg.io/media/games/742/7424c1f7d0a8da9ae29cd866f985698b.jpg",
      games: [
        {
          id: 23557,
          slug: "gwent-the-witcher-card-game",
          name: "Gwent: The Witcher Card Game",
          added: 4734,
        },
        {
          id: 327999,
          slug: "dota-underlords",
          name: "Dota Underlords",
          added: 3999,
        },
        {
          id: 2055,
          slug: "adventure-capitalist",
          name: "AdVenture Capitalist",
          added: 3359,
        },
        {
          id: 758,
          slug: "hue",
          name: "Hue",
          added: 2487,
        },
        {
          id: 2306,
          slug: "poker-night-2",
          name: "Poker Night 2",
          added: 2042,
        },
        {
          id: 3187,
          slug: "armello",
          name: "Armello",
          added: 2009,
        },
      ],
    },
    {
      id: 34,
      name: "Educational",
      slug: "educational",
      games_count: 15675,
      image_background:
        "https://media.rawg.io/media/screenshots/6cd/6cd13ed3dcb6b44b8bc995850f2861e6.jpg",
      games: [
        {
          id: 1358,
          slug: "papers-please",
          name: "Papers, Please",
          added: 6784,
        },
        {
          id: 1140,
          slug: "world-of-goo",
          name: "World of Goo",
          added: 4389,
        },
        {
          id: 2778,
          slug: "surgeon-simulator-cpr",
          name: "Surgeon Simulator",
          added: 3891,
        },
        {
          id: 9768,
          slug: "gameguru",
          name: "GameGuru",
          added: 2543,
        },
        {
          id: 13777,
          slug: "sid-meiers-civilization-iv-colonization",
          name: "Sid Meier's Civilization IV: Colonization",
          added: 2304,
        },
        {
          id: 6885,
          slug: "pirates-3",
          name: "Sid Meier's Pirates!",
          added: 2231,
        },
      ],
    },
    {
      id: 17,
      name: "Card",
      slug: "card",
      games_count: 4526,
      image_background:
        "https://media.rawg.io/media/games/431/4317e294e88e4c9d77327693b15f499a.jpg",
      games: [
        {
          id: 28121,
          slug: "slay-the-spire",
          name: "Slay the Spire",
          added: 4805,
        },
        {
          id: 23557,
          slug: "gwent-the-witcher-card-game",
          name: "Gwent: The Witcher Card Game",
          added: 4734,
        },
        {
          id: 18852,
          slug: "poker-night-at-the-inventory",
          name: "Poker Night at the Inventory",
          added: 2721,
        },
        {
          id: 332,
          slug: "the-elder-scrolls-legends",
          name: "The Elder Scrolls: Legends",
          added: 2146,
        },
        {
          id: 8923,
          slug: "faeria",
          name: "Faeria",
          added: 2147,
        },
        {
          id: 2306,
          slug: "poker-night-2",
          name: "Poker Night 2",
          added: 2042,
        },
      ],
    },
  ],
};

const carouselContainer = document.getElementById('carousel-container');

$(document).ready(function () {
  // Itera sobre los resultados de la API y genera el HTML dinámicamente
  vectorImg = []
  data.results.forEach(category => {
      const indice = categorys.indexOf(category.name);
      rutaImg = `../images/${categorys[indice].replace(/\s+/g, '_').toLowerCase()}.svg`;
      let idGame=""
      const url2 = `https://api.rawg.io/api/games/${idGame}?key=0d5c200a4d584e4bb294c6e20858da87`
      // Crear y añadir el título de la categoría
      const titleElement = document.createElement('h2');
      const titleImgElement = document.createElement('img');
      
      /* titleImgElement.src = '../images/icon-category.svg';
      titleElement.classList.add('titleIcon');
      titleImgElement.src = rutaImg;
      titleImgElement.alt = categorys[indice];
      titleElement.appendChild(titleImgElement);
      titleElement.appendChild(document.createTextNode(category.name));

      carouselContainer.appendChild(titleElement);

      // Crear y añadir el contenedor del carrusel
      const owlCarouselElement = document.createElement('div');
      owlCarouselElement.classList.add('owl-carousel', 'owl-theme');
      
      category.games.forEach(game => {
          const figureElement = document.createElement('figure');
          
          const imgElement = document.createElement('img');
          obtenerDatosDeJuegos(game.id)
          .then(data => {
            imgElement.src = data.background_image;
            imgElement.alt = game.name;
            imgElement.classList.add('slide');
          })
          .catch(error => {
            console.error('Error final:', error);
          });
          
          const figcaptionElement = document.createElement('figcaption');
          figcaptionElement.textContent = game.name;

          const anchorElement = document.createElement('a');
          anchorElement.href = `view-game.html?game=${game.slug}`;
          anchorElement.classList.add('btn-more');

          const imgMoreElement = document.createElement('img');

          anchorElement.appendChild(imgMoreElement);
          anchorElement.appendChild(document.createTextNode('view'));

          figureElement.appendChild(imgElement);
          figureElement.appendChild(figcaptionElement);
          figureElement.appendChild(anchorElement);

          owlCarouselElement.appendChild(figureElement);
      });

      carouselContainer.appendChild(owlCarouselElement);
  });
}); */
